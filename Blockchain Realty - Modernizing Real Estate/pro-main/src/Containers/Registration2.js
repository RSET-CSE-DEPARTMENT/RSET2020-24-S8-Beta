...
class Register extends Component {
  ...
  //     const pinataRes = await axios.post(    "https://api.pinata.cloud/pinning/pinJSONToIPFS",
  //               formData,
  //               {
  //                 headers: {
...
const pinataMetadata = JSON.stringify({
  name: 'Name of your file', // Changed blob.name to a static name
});
formData.append("pinataMetadata", pinataMetadata);
  ...
  handleSubmit = async () => {
    ...
    if (data) {
        try {
            ...
            await this.Register(data, account, laddress, lamount);
        } catch (error) {
            console.log('error:', error);
        }
    } else {
