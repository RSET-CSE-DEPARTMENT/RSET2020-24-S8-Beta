  ...
  handleSubmit = async () => {
    ...
    if (this.state.address) {
      try {
        ...
        // Check if the entered private key matches the main account in MetaMask
        if (this.state.address.toLowerCase() === mainAccount.toLowerCase()) {
          const user = await this.state.landList.methods.getUser(data.address).call();
          ...
          if (user) {
            ...
            if (this.state.exist) {
              ...
              console.log('Login Failed');
              window.alert('Login Failed: Invalid private key.');
              window.localStorage.setItem('authenticated', false);
            }
          } else {
            console.log('Please try again');
