...
class Register extends Component {
  ...
  checkPrivateKey = () => {
    return this.state.address === this.state.account
  ...
  handleSubmit = async () => {
    ...
    if (
      ...
      if (this.validateEmail(this.state.email)) {
        if (this.checkPrivateKey()) { // Check if private key matches the current MetaMask account
            ...
            (response) => {
              if (response.status == 200) {
                this.setState({
                  ...
                  city: '',
                  contact: '',
                 // privateKey: '', // Reset private key after successful signup
                })
                this.login(data) // Call login function after successful signup
              }
            },
            (error) => {
              ...
              this.setState({
                ...
                city: '',
                contact: '',
                //privateKey: '', // Reset private key after error
              })
            },
          )
        } else {
          alert('Private key does not match the current MetaMask account')
          window.alert(this.state.privateKey);
          window.alert(this.state.account); // Display error message
        }
      } else {
